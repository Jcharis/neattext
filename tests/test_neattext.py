...
def test_remove_currencies():
    ...
    assert (
        str(result)
        == "This is the mail example@gmail.com ,our WEBSITE is https://example.com 😊 and it will cost  to subscribe."
    )
...
def test_remove_currency_symbols():
    ...
    assert (
        str(result)
        == "This is the mail example@gmail.com ,our WEBSITE is https://example.com 😊 and it will cost 100 to subscribe."
    )
...
def test_textframe_remove_puncts():
    ...
    assert (
        result.text
        == "This is the mail example@gmailcom our WEBSITE is https://examplecom 😊"
    )
...
def test_single_fxn_clean_text_custom_pattern():
    ...
    assert (
        result == "this is the mail example .com ,our website is https://example.com ."
    )
...
def test_convert_text_case_basic():
    text = "convert this text"
    assert convert_text_case(text, TextCase.UPPER) == "CONVERT THIS TEXT"
    assert convert_text_case(text, TextCase.lower) == "convert this text"
    assert convert_text_case(text, TextCase.Title) == "Convert This Text"


def test_convert_text_case():
    text = "convert this text"
    assert convert_text_case(text, TextCase.snake) == "convert_this_text"
    assert convert_text_case(text, TextCase.camel) == "convertThisText"
    assert convert_text_case(text, TextCase.pascal) == "ConvertThisText"
    assert convert_text_case(text, TextCase.kebab) == "convert-this-text"


def test_convert_text_case_from_to():
    text = "convert this text"
    assert snake_to_pascal("convert_this_text") == "ConvertThisText"
    assert snake_to_camel("convert_this_text") == "convertThisText"
    assert to_pascal_case("convert this text") == "ConvertThisText"
